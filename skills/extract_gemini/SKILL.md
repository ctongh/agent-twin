...
| File | Purpose |
|------|---------|
| `personalized/saves/session/<session_id>/conversation.json` | Raw turn-by-turn capture |
| `personalized/saves/session/<session_id>/annotated.txt` | Same content with topic-cluster headers |
...
```
personalized/saves/session/<session_id>/
```
...
The hex GUID makes collision essentially impossible, but a re-extract on the same day can collide if the user reuses an ID intentionally. Before writing into an existing `personalized/saves/session/<session_id>/`:
...
```bash
mv ~/Downloads/gemini-conversation.json personalized/saves/session/<session_id>/conversation.json
```
...
```python
import json, sys
with open('personalized/saves/session/<session_id>/conversation.json') as f:
    data = json.load(f)
