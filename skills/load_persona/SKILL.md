...
When invoked, read the behavior brief at `personalized/results/profile/behavior_brief.md` and from that point forward let it shape every response in the session.

This skill is invoked **explicitly** — the persona must not auto-load, so the before/after difference is perceptible.

## What this skill reads

A single file: `personalized/results/profile/behavior_brief.md`.
...
Read the file at `personalized/results/profile/behavior_brief.md`.
...
If the file exists but is older than the most recent session under `personalized/saves/session/`, surface a soft warning:
...
Do **not** print the brief contents to the conversation. The brief is operational context, not a document to review. If the user wants to inspect it, they can read `personalized/results/profile/behavior_brief.md` directly.
